...
class PayPalService
{
    ...
    public function setItem($itemData)
    {
        ...
        foreach ($itemData as $data) {
            ...
            // Set tên của item
            $item->setName($data['product_name'])
                 ->setCurrency($this->paymentCurrency) // Đơn vị tiền của item
                 //->setSku($data['product_id']) // ID của item
    ...
    public function createPayment($transactionDescription)
    {
        ...
        foreach ($payment->getLinks() as $link) {
            ...
            if ($link->getRel() == 'approval_url') {
                ...
                //Session::push('paypal_payment_id',$payment->getId());
    ...
    public function getPaymentStatus()
    {
        ...
        $payment = Payment::get(array_values($request)[0], $this->apiContext);
